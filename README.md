...
prediction intervals — split-conformal calibrated, exact in distribution and **worse than no
calibration under every distribution shift tested** — and on operational quiescent-window detection
rather than on RMSE alone. **Every result in this repository comes
...
- **Both over-cover, and these are the *uncalibrated* heads.** The bands are labelled 90 % and
  measure 100 % and 99 % on this span; over the `id` test partition at this same channel and lead
  they are 0.961 and 0.984 against a 0.90 nominal. The uncalibrated heads are calibrated at
  10–15 s and systematically too wide across the 1–5 s band the landing decision is actually taken
  in — in-band cells run 5, 5, 7, 19, 30, 36 of 36 from 1 s to 15 s. [Split conformal
  fixes exactly this](#calibration-split-conformal-is-exact-in-distribution-and-harmful-under-shift)
  in distribution; the figure is drawn from the committed uncalibrated trace and is not re-rendered
  from the calibrated arm.
...
**In distribution it works exactly — which is a correctness check, not a discovery.** All 216 cells
land in band and at the gate cell the six heads sit between 0.8989 and 0.9025. That is the CQR
guarantee arriving on schedule; had it *not* happened the implementation would be wrong, and Gate 10
predicate 4 reads it that way. The interesting half is not guaranteed: the intervals also come out
**17.5% sharper**, because the uncalibrated heads were over-covering (median 0.945 against a nominal
0.90) and paying for it in width, so calibration recovered width rather than costing it.
...
uncalibrated rows keep the real number. And the finite-sample term is nominal: the 23 267–24 677
windows actually drawn sit 0.5 s apart on a ~12 s roll period, so they are far fewer than that many
*independent* samples, and the binding uncertainty is the realization bootstrap interval rather than
`1/(n+1)`. The calibration table carries a realization-level scale beside `gamma` for exactly this
reason — 0.43–0.45 against 0.77–0.82 — though it certifies a weaker target and no claim here rests
on it.

**What the proper scores say, and what is post-hoc.** CRPS and pinball agree in sign with Winkler
in all four regimes (median change: `id` −1.2%, and +2.8 / +1.5 / +1.4% under shift), so the
conclusion does not rest on a choice of scoring rule. But only the in-band counts and median PICP
were pre-registered; "cells that got worse", the width change and the Winkler change are **post-hoc
statistics**. All three cut against the arm, so the post-hoc status buys nothing here — it is stated
rather than left to be noticed.

**`unseen_heading`'s shift is small cell by cell.** Its median |ΔPICP| (0.031) sits below the median
pooled seed spread (0.034), so no single cell there is individually decisive. The direction is
robust anyway because the contrast is **paired** — same checkpoints, same windows, only the
post-processing differs — and the cells-worse fraction is 77.3 / 72.7 / 78.2% across the three seeds
taken separately.

Full tables: [`results/e05/`](results/e05/). Pre-registration and the scored predictions are P10-D1
and P10-D2 in [`docs/protocol.md`](docs/protocol.md): **two of four magnitude predictions were
falsified**, a third (`unseen_vessel`) landed one cell outside its predicted interval, and the
ordering held. **P10-D1 also promised a second, additive arm that was never built** — an omission
rather than a decision, recorded in P10-D4. Gate 10 passes 7/7; its first predicate verifies the
committed uncalibrated tables carry no uncommitted modification, and they are byte-identical to
`main`.
...
matched-origin arms, assemble and render `results/results.md`, read Gates 4–7, calibrate the
interval heads and read Gate 10, run the Phase 8 cross-validation, and regenerate the figures. It
totals **about 167 hours on one RTX A4000** — call it a week. This is not an afternoon:
...
| split-conformal calibration, 4 regimes x 18 checkpoints | `make conformal` | ~3.50 h | protocol prose (P10-D3) |
| Gate 10 read-out | `make gate10` | seconds | reads committed CSVs only |
...
the committed record. `make conformal` postdates that convention but its first run predates the
status file it now writes, so its 3.50 h is prose for this run and logged for the next. The e02 and
e03 sweeps predate the convention entirely and their figures come from
